...
This is a sample template for pool-monitor - Below is a brief explanation of what we have generated for you:
...
```bash
sam logs -n HelloWorldFunction --stack-name pool-monitor --tail
```
...
```bash
aws cloudformation delete-stack --stack-name pool-monitor
```
